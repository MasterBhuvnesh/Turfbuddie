...
//         <div className="m-28 max-w-4xl mx-auto bg-gray-800 rounded-lg shadow-lg overflow-hidden">
...
//                 ))
//             ) : (
//                 <div className="relative h-64 bg-gray-800 flex items-center justify-center rounded-lg col-span-full">
...
//     <table className="min-w-full bg-gray-800 border-separate border-spacing-2">
//       <thead>
...
//           <tr key={hour}>
//             {Object.keys(slotsByDate).map((date) => {
//               const slotTime = `${String(hour).padStart(2, "0")}:00 - ${String(
//                 hour + 1
//               ).padStart(2, "0")}:00`;
...
//                 <td
//                   key={date}
//                   className={`px-4 py-2 text-center rounded-lg whitespace-nowrap ${
...
export default async function TurfPage({ params }: { params: Promise<PageProps["params"]> }) {
  ...
  const { data: bookingSlots, error: slotsError } = await supabase
    ...
    .lte("date", new Date(new Date().setDate(new Date().getDate() + 5)).toISOString().split("T")[0])
    .order("date", { ascending: true })
    .order("start_time", { ascending: true });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-green-200 to-green-500 p-4">
      <div className="m-28 max-w-4xl mx-auto bg-gray-800 rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="p-4">
          {turf.images && turf.images.length > 0 ? (
            <ImageSlider images={turf.images} turfName={turf.name} />
          ) : (
            <div className="relative h-64 bg-gray-800 flex items-center justify-center rounded-lg col-span-full">
              <p className="text-white">No images available</p>
            </div>
        ...
        {/* Turf Details */}
        <div className="p-4 md:p-6">
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-4">{turf.name}</h1>
          <p className="text-white mb-2">
            <span className="font-semibold">Location:</span> {turf.location}
          </p>
          <p className="text-white mb-2">
     ...
     <table className="min-w-full bg-gray-800 border-separate border-spacing-2">
      ...
      <tbody>
        {Array.from({ length: 24 }).map((_, hour) => (
          ...
          <p className="text-xl md:text-2xl font-bold text-white mb-4">Price: ₹{turf.price} per hour</p>
          <BookSlot turfId={turf.id} />
          <button className="w-full bg-blue-500 text-white py-2 md:py-3 rounded-md hover:bg-blue-600 transition-colors font-semibold">
            Pay Now
          </button>
